<script lang="ts">
    ...
    unsubs.push(
      wsStore.on('AgentStateChange', (msg: WsMessage) => {
        ...
        addNotification({
          ...
          message: `New state: ${typeof msg.status === 'string' ? msg.status : typeof msg.new_state === 'string' ? msg.new_state : 'unknown'}`,
      ...
      wsStore.on('KillSwitchActivation', (msg: WsMessage) => {
        addNotification({
          ...
          message: typeof msg.reason === 'string' ? msg.reason : 'No reason provided',
          actionHref: '/security',
        });
      }),
      wsStore.on('InterventionChange', (msg: WsMessage) => {
        ...
        addNotification({
          ...
          message: `Agent ${agentId || 'unknown'}: level → ${msg.new_level ?? 'unknown'}`,
          actionHref: '/convergence',
          agentId: agentId || undefined,
        });
      }),
      wsStore.on('ProposalUpdated', (msg: WsMessage) => {
        const proposalId = typeof msg.proposal_id === 'string' ? msg.proposal_id : '';
        const change = typeof msg.change === 'string' ? msg.change : 'updated';
        const status = typeof msg.status === 'string' ? msg.status : 'updated';
        addNotification({
  ...
  function persistToStorage() {
    ...
    try {
      ...
      // Ignore storage persistence failures and keep in-memory notifications working.
    }
  }
</script>
