            ...
            <div class="provider-actions">
              {#if editingEnv === p.env_name}
                <div class="key-form">
                  ...
                  />
                  <button type="button" class="save-btn" disabled={saving || !keyInput.trim()} onclick={() => saveKey(p.env_name)}>
                    {saving ? '...' : 'Save'}
                  </button>
                  <button type="button" class="cancel-btn" onclick={cancelEditing}>Cancel</button>
                </div>
              {:else}
                <button type="button" class="set-btn" onclick={() => startEditing(p.env_name)}>
                  {p.is_set ? 'Update Key' : 'Set Key'}
                </button>
                {#if p.is_set}
                  <button type="button" class="delete-btn" onclick={() => removeKey(p.env_name)}>Remove</button>
                {/if}
              {/if}
            </div>
          {:else if p.provider_name === 'codex'}
            <div class="provider-actions">
              {#if codexStatus?.account}
                <button type="button" class="cancel-btn" disabled={codexBusy || codexLoading} onclick={refreshCodexStatus}>
                  {codexLoading ? '...' : 'Refresh'}
                </button>
                <button type="button" class="delete-btn" disabled={codexBusy} onclick={logoutCodex}>
                  {codexBusy ? '...' : 'Disconnect'}
                </button>
              {:else}
                <button type="button" class="set-btn" disabled={codexBusy || codexLoading} onclick={startCodexLogin}>
