...
fn cortex_temporal_uses_blake3_not_sha2() {
    ...
    // cortex-temporal should not depend on sha2 for its own hashing
    // (it may transitively pull it in, but should not use it directly)
}
...
fn ghost_signing_is_leaf_crate() {
    ...
    // Check that [dependencies] section doesn't contain ghost- or cortex- crates
