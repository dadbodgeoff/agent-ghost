...
impl Skill for KeyboardPressSkill {
    fn name(&self) -> &str { "keyboard_press" }
    ...
    fn execute(&self, ctx: &SkillContext<'_>, input: &serde_json::Value) -> SkillResult {
        ...
        if let Some(app) = target_app {
            if let ValidationResult::Denied(reason) = self.validator.validate_app(app) {
                audit::log_blocked_action(ctx.db, ctx.agent_id, ctx.session_id, "keyboard_press", input, &reason);
                return Err(SkillError::PcControlBlocked(reason));
            }
        }

        // Safety: circuit breaker.
        { self.circuit_breaker.lock().unwrap().check("keyboard_press")?; }
        ...
        audit::log_pc_action(ctx.db, ctx.agent_id, ctx.session_id, "keyboard_press", input, &result);
...
mod tests {
    ...
    fn presses_return_key() {
        ...
        let result = skill.execute(&ctx, &serde_json::json!({"key": "Return"})).unwrap();
    ...
    fn presses_single_character() {
        ...
        let result = skill.execute(&ctx, &serde_json::json!({"key": "a"})).unwrap();
        assert_eq!(result["status"], "ok");
    ...
    fn presses_function_key() {
        ...
        let result = skill.execute(&ctx, &serde_json::json!({"key": "F5"})).unwrap();
        assert_eq!(result["status"], "ok");
    ...
    fn blocks_disallowed_app() {
        ...
        let result = skill.execute(&ctx, &serde_json::json!({"key": "Return", "target_app": "Terminal"}));
