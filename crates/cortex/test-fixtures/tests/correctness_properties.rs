...
proptest! {
    ...
    fn amplified_score_bounded(
        ...
        let scorer = CompositeScorer::default();
        let baseline = BaselineState::default();
        let meso_data: Vec<f64> = if meso { vec![0.3, 0.4, 0.5] } else { vec![] };
        let macro_data: Vec<f64> = if macro_amp { vec![0.6, 0.7, 0.8] } else { vec![] };
        let meso_ref = if meso { Some(meso_data.as_slice()) } else { None };
        let macro_ref = if macro_amp { Some(macro_data.as_slice()) } else { None };
        let result = scorer.score(&signals, &baseline, meso_ref, macro_ref);
        prop_assert!(
            (0.0..=1.0).contains(&result.score),
