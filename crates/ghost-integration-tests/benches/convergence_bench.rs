...
fn bench_simulation_boundary_scan(c: &mut Criterion) {
    ...
    c.bench_function("boundary_scan_clean_500chars", |b| {
        b.iter(|| enforcer.scan_output(black_box(clean_text), EnforcementMode::Hard));
    });
    ...
    c.bench_function("boundary_scan_violation", |b| {
        b.iter(|| enforcer.scan_output(black_box(violation_text), EnforcementMode::Hard));
    });

    // Large text (10KB)
    let large_text = clean_text.repeat(20);
    c.bench_function("boundary_scan_10kb", |b| {
        b.iter(|| enforcer.scan_output(black_box(&large_text), EnforcementMode::Hard));
    });
}
...
fn bench_proposal_validation(c: &mut Criterion) {
    ...
    let ctx = ProposalContext {
        ...
        caller: CallerType::Agent { agent_id },
    };
