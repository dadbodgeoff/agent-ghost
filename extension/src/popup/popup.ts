...
function updateUI(data: { score: number; level: number; signals: number[] }): void {
  ...
  if (signalListEl) {
    signalListEl.innerHTML = data.signals
      .map(
        (val, index) =>
          `<div class="signal-row"><span class="signal-name">Signal ${index + 1}</span><span class="signal-value">${val.toFixed(2)}</span></div>`
      )
      .join('');
  }
  ...
  if (platformEl) {
    platformEl.textContent = chrome.runtime.getManifest().name;
  }
}
