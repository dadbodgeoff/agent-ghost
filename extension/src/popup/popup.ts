      ...
      .map((value, index) => {
        ...
        return `
          <div class="signal-row">
            <span class="signal-name">${labels[index]}</span>
            <span class="signal-value">${value.toFixed(2)}</span>
            <span class="signal-bar" aria-hidden="true">
              <span class="signal-bar-fill" style="width:${pct}%;background:${resolveSignalColor(value)};"></span>
            </span>
          </div>
        `;
      })
...
function resolveSignalColor(value: number): string {
...
(async () => {
  ...
  if (platformEl) {
    try {
      platformEl.textContent = new URL(auth.gatewayUrl).hostname || 'Gateway';
    } catch {
