    ...
    signalListEl.innerHTML = data.signals
      .map((value, index) => {
        ...
        return (
          ...
          `<span class="signal-bar-fill" style="width:${percentage}%;background:${percentage >= 70 ? '#ef4444' : percentage >= 40 ? '#f59e0b' : '#22c55e'}"></span>` +
          `</span>` +
          `</div>`
        );
      })
...
function updateSessionDuration(): void {
  ...
  if (timerEl) timerEl.textContent = `${elapsedMinutes}m`;
}
