...
fn normalize_terminal_dimension(value: u16) -> u16 {
    value.max(MIN_TERMINAL_DIMENSION)
}
...
pub async fn open_terminal_session<R: tauri::Runtime>(
    ...
    let pair = pty_system
        .openpty(PtySize {
            rows: normalize_terminal_dimension(rows),
            cols: normalize_terminal_dimension(cols),
            pixel_width: 0,
            pixel_height: 0,
        })
    ...
    std::thread::spawn(move || {
        ...
        if let Some(terminal_state) = app_handle_clone.try_state::<DesktopTerminalState>() {
            if let Ok(mut sessions) = terminal_state.sessions.write() {
...
pub async fn close_terminal_session(
    ...
    let Some(session) = take_session(&terminal_state, session_id)? else {
        return Ok(());
    };
    session
        .child_killer
