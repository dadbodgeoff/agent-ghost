...
fn update_gateway_port(handle: &AppHandle, port: u16) {
    if let Some(state) = handle.try_state::<GatewayPort>() {
        state.0.store(port, Ordering::Relaxed);
    }
}

async fn replace_gateway_process(handle: &AppHandle, next_child: Option<CommandChild>) {
...
pub async fn auto_start(handle: AppHandle) -> Result<(), GhostDesktopError> {
    ...
    update_gateway_port(&handle, port);
    ...
    // Store child handle for shutdown without re-registering app state.
